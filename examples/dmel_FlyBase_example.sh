...
GENOME="dmel_FlyBase_chr4.fa"

# ANNOATION in gff3 FORMAT
ANNOTATION="dmel_FlyBase_chr4.gff3"
...
echo -e "Running the following command:\n$EMBLmyGFF3 --rg $REFERENCE_GROUP -a $ACCESSION -p $PROJECT -l \"$CLASSIFICATION\" -m \"$MOLECULE\" -d $CLASS -r $TABLE -t linear -s \"$SPECIES\" -x $TAXONOMY $ANNOTATION $GENOME $@ > dmel_FlyBase_chr4.embl"
